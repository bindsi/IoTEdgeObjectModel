        ...
        public static async Task<string> GetDeploymentManifest(this RegistryManager registryManager, string deviceId)
		{
			...
			DeploymentManifest manifest = new DeploymentManifest()
			{
				ModulesContent = new ModulesCtnt()
				{
					...
					{
                        Properties = new PropertiesEdgeAgent()
						{
							Desired = JsonConvert.DeserializeObject<DesiredEdgeAgent>(edgeAgentDesiredProperties),
						},
					},
					EdgeHub = new EdgeHub()
					{
						Properties = new PropertiesEdgeHub()
						{
							Desired = JsonConvert.DeserializeObject<DesiredEdgeHub>(edgeHubDesiredProperties),
						},
					},
				},
			};
